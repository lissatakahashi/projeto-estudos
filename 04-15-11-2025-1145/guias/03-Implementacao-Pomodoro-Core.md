...
- `tcc-prototype-web/src/domain/pomodoro/types/PomodoroHistoryItem.ts` — tipo `PomodoroHistoryItem` com `pomodoroHistoryItemId: PomodoroHistoryItemId`.
   ...
   Observação: cada arquivo em `src/domain/pomodoro/types/` inclui comentários
   inline que documentam propósito, campos e exemplos de uso. Os tipos que
   descrevem valores literais/enumeradores (`PomodoroMode`, `PomodoroStatus`) foram
   organizados separadamente em `src/domain/pomodoro/types/enums/` para deixá-los
   claros e fáceis de localizar. Consulte esses arquivos para entender contratos
   e convenções de nomes (por exemplo `pomodoroId`).
